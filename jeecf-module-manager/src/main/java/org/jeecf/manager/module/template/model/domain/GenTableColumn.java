...
public class GenTableColumn extends BaseEntity implements Serializable{
	...
	@NotBlank(message="列名输入不能为空",groups= {Add.class})
	@Length(min = 1, max = 20, message = "列名长度必须介于 1 和 20 之间",groups={Add.class})
	@Pattern(regexp="^[a-zA-Z]+[a-zA-Z_]*[a-zA-Z]$",message="列名只能由a-zA-Z_组成",groups= {Add.class})
	...
	@NotBlank(message="解释输入不能为空",groups= {Add.class})
	@Length(min = 1, max = 50, message = "解释长度必须介于 1 和 50 之间",groups={Add.class})
	...
	@NotNull(message="输入不能为空",groups= {Add.class})
	@Length(min = 1, max = 50, message = "jdbc类型长度必须介于 0 和 50 之间",groups={Add.class})
	...
	@Min(value=0,message="输入错误",groups= {Add.class})
	@Max(value=9,message="输入错误",groups= {Add.class})
	...
	@NotBlank(message="属性输入不能为空",groups= {Add.class})
	@Length(min = 1, max = 50, message = "属性长度必须介于 0 和 50 之间",groups={Add.class})
	@Pattern(regexp="^[a-zA-Z]+$",message="属性只能由a-zA-Z组成",groups= {Add.class})
