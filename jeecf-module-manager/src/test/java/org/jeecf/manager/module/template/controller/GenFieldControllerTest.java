...
public class GenFieldControllerTest extends BaseMokMvc {
    ...
    public void list() throws Exception {
        ...
        String responseString = mockMvc.perform(MockMvcRequestBuilders.post("/template/genField/list").contentType(MediaType.APPLICATION_JSON).content(requestJson))
    ...
    public void column() throws Exception {
        String genFieldId = "1";
        String responseString = mockMvc.perform(MockMvcRequestBuilders.post("/template/genField/column/" + genFieldId).contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk()).andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
        assert JsonMapper.getJsonNode(responseString).get(SUCCESS).asBoolean();
    }
    ...
    private String save(GenField genField) throws Exception {
        ...
        return mockMvc.perform(MockMvcRequestBuilders.post("/template/genField/save").contentType(MediaType.APPLICATION_JSON).content(requestJson)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
    }

    private String delete(String id) throws Exception {
        return mockMvc.perform(MockMvcRequestBuilders.post("/template/genField/delete/" + id).contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
    }
