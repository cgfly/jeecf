...
public class SysTableDictControllerTest extends BaseMokMvc {
    ...
    public void list() throws Exception {
        ...
        String responseString = mockMvc.perform(MockMvcRequestBuilders.post("/gen/sysTableDict/list").contentType(MediaType.APPLICATION_JSON).content(requestJson))
    ...
    private String save(SysTableDict sysTableDict) throws Exception {
        ...
        return mockMvc.perform(MockMvcRequestBuilders.post("/gen/sysTableDict/save").contentType(MediaType.APPLICATION_JSON).content(requestJson)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
    }

    private String delete(String id) throws Exception {
        return mockMvc.perform(MockMvcRequestBuilders.post("/gen/sysTableDict/delete/" + id).contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
    }

    public void tables() throws Exception {
        String responseString = mockMvc.perform(MockMvcRequestBuilders.post("/gen/sysTableDict/tables").contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
        assert JsonMapper.getJsonNode(responseString).get(SUCCESS).asBoolean();
    }
