...
public class SysMenuControllerTest extends BaseMokMvc {
    ...
    public void list() throws Exception {
        ...
        String responseString = mockMvc.perform(MockMvcRequestBuilders.post("/config/sysMenu/list").contentType(MediaType.APPLICATION_JSON).content(requestJson))
    ...
    public void getTreeData() throws Exception {
        ...
        String responseString = mockMvc.perform(MockMvcRequestBuilders.post("/config/sysMenu/getTreeData").contentType(MediaType.APPLICATION_JSON).content(requestJson))
                .andExpect(MockMvcResultMatchers.status().isOk()).andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
        assert JsonMapper.getJsonNode(responseString).get(SUCCESS).asBoolean();
    }
    ...
    private String save(SysMenu sysMenu) throws Exception {
        ...
        return mockMvc.perform(MockMvcRequestBuilders.post("/config/sysMenu/save").contentType(MediaType.APPLICATION_JSON).content(requestJson)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
    }

    private String delete(String id) throws Exception {
        return mockMvc.perform(MockMvcRequestBuilders.post("/config/sysMenu/delete/" + id).contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
    }
