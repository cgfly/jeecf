...
public class SysNamespaceControllerTest extends BaseMokMvc {
    ...
    public void list() throws Exception {
        ...
        String responseString = mockMvc.perform(MockMvcRequestBuilders.post("/config/sysNamespace/list").contentType(MediaType.APPLICATION_JSON).content(requestJson))
    ...
    public void effect() throws Exception {
        String id = "1";
        String responseString = mockMvc.perform(MockMvcRequestBuilders.post("/config/sysNamespace/effect/" + id).contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk()).andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
        assert JsonMapper.getJsonNode(responseString).get(SUCCESS).asBoolean();
    }
    ...
    private String save(SysNamespace sysNamespace) throws Exception {
        ...
        return mockMvc.perform(MockMvcRequestBuilders.post("/config/sysNamespace/save").contentType(MediaType.APPLICATION_JSON).content(requestJson)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
    }

    private String delete(String id) throws Exception {
        return mockMvc.perform(MockMvcRequestBuilders.post("/config/sysNamespace/delete/" + id).contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk())
                .andDo(MockMvcResultHandlers.print()).andReturn().getResponse().getContentAsString();
    }
